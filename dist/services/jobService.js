    ...
    async executeFuncForAllAlgos(algoState) {
        ...
        for (let sortFunc of ARRAY_OF_SORT_FUNCTIONS) {
            for (let arraySize of ARRAY_SIZES) {
                const interRes = await this.executeFuncForString(algoState, sortFunc.name, arraySize);
                res.push(interRes);
            }
        }
    ...
    async checkBdForData() {
        ...
        SORT_TYPES.forEach(sortType => {
            ARRAY_SIZES.forEach(arraySize => {
                let exist = dataForCycle.some((item) => item.sortType.toUpperCase() === sortType.toUpperCase() && item.arraySize === arraySize);
                if (!exist) {
        ...
        if (missingData.length > 0 || duplicates.length > 0) {
            ...
            return `База данных заполнена полностью — все сортировки и размеры массивов: ${JSON.stringify(Object.fromEntries(result))}`;
        }
    }
}
